...
router.post('/generate', authenticateToken, async (req, res) => {
    try {
        ...
        // Build preview object for frontend live display
        const preview = {
            personalInfo: {
                name: cvContent.header?.name || '',
                email: cvContent.header?.email || '',
                phone: cvContent.header?.phone || '',
                country: cvContent.header?.country || '',
                profilePicture: cvContent.header?.profilePicture || ''
            },
            summary: cvContent.summary || '',
            education: cvContent.education || [],
            skills: cvContent.skills || [],
            languages: Array.isArray(cvContent.languages) ? cvContent.languages.map(l => l.language || l) : []
        };
...
// Simple CV download (auto-generate if needed)
router.get('/download', authenticateToken, async (req, res) => {
    try {
        const db = getDb(req);
        
        // Quick fix: If no database, return demo CV
        if (!db || !db.query) {
            console.log('⚠️ QUICK FIX: No database connection, returning demo CV');
            const demoCV = generateDemoCV();
            
            res.setHeader('Content-Disposition', 'attachment; filename="JobScooter_Demo_CV.html"');
            res.setHeader('Content-Type', 'text/html');
            return res.send(demoCV);
        }
        
        // Check if user has an existing CV
        let existingCV = await new Promise((resolve, reject) => {
            db.query(`
        ...
        // If no existing CV or file doesn't exist, generate new one
        if (!existingCV || !fs.existsSync(existingCV.file_path)) {
            console.log('Generating new CV for user:', req.user.id);
            
            // Get user data and generate CV
            const userData = await getUserCompleteProfile(req.user.id, db);
            if (!userData) {
                return res.status(404).json({ error: 'User profile not found' });
            }
            
            const cvContent = await generateCVContent(userData, 'professional');
            const htmlCV = generateHTMLCV(cvContent, 'professional');
            
            // Generate filename and path
            const cvId = uuidv4();
            const filename = `cv_${req.user.id}_${cvId}.html`;
            const cvPath = path.join('./public/uploads/cvs', filename);
            ...
            await new Promise((resolve, reject) => {
                db.query(`
                    INSERT INTO generated_cvs (
                        applicant_id, cv_id, template_type, format, file_path, 
                        file_url, generation_data, created_at, updated_at
                    ) VALUES (?, ?, 'professional', 'html', ?, ?, ?, NOW(), NOW())
                `, [
                    req.user.id, cvId, cvPath, `/uploads/cvs/${filename}`,
                    JSON.stringify(cvContent)
                ], (err, result) => {
                    if (err) reject(err);
                    else resolve(result);
        ...
        // Download the CV
        const filename = `JobScooter_CV_${existingCV.template_type}.${existingCV.format}`;
        res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
        res.setHeader('Content-Type', 'text/html');
        
        const fileStream = fs.createReadStream(existingCV.file_path);
        fileStream.pipe(res);
...
// Generate a demo CV for users without database access
function generateDemoCV() {
    const demoContent = {
        header: {
            name: 'Demo User',
        ...
        summary: 'Experienced professional with verified qualifications through JobScooter\'s AI-powered verification system, achieving Green status for comprehensive credentials.',
        education: [
            {
        ...
        certifications: [
            ...
            {
                name: 'German Language Certificate B2',
        ...
        skills: ['Software Development', 'Computer Science', 'German (B2)', 'English (Native)', 'Professional Certification'],
        trafficLightStatus: {
            status: 'green',
            score: 85
        }
    ...
    return generateHTMLCV(demoContent, 'professional');
}
