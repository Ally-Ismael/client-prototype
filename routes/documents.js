...
router.post('/manual-entry', (req, res) => {
    try {
        ...
        // 1) { first_name, surname, id_number, country?, email?, phone? }
        // 2) { manual_data: { firstName, surname, idNumber, dateOfBirth?, address? }, session_token? }
        let first_name, surname, id_number, country, email, phone, date_of_birth, address;

        if (req.body && req.body.manual_data) {
            const md = req.body.manual_data || {};
            first_name = md.firstName || md.first_name;
            surname = md.surname;
            id_number = md.idNumber || md.id_number;
            date_of_birth = md.dateOfBirth || md.date_of_birth;
            address = md.address;
            // country/email/phone may be provided later on the additional info step
            country = md.country || 'South Africa';
            email = md.email;
            phone = md.phone;
        } else {
            ({ first_name, surname, id_number, country, email, phone } = req.body || {});
        }

        // Validate minimally for this step (collect contact info later)
        if (!first_name || !surname || !id_number) {
            return res.status(400).json({
                success: false,
                error: 'Missing required fields',
                required: ['first_name', 'surname', 'id_number']
            });
        }
