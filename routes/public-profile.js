...
// Get current user's own profile (for live viewing)
router.get('/my-profile', async (req, res) => {
    try {
        ...
        try {
            const decoded = jwt.verify(token, process.env.JWT_SECRET || 'jobscooter_development_secret_key_2024');
            userId = decoded.id;
        } catch (error) {
            return res.status(401).json({
                error: 'Invalid token'
        ...
        const languages = await new Promise((resolve, reject) => {
            db.query(`
                SELECT language, is_verified, 
                       certificate_type, proficiency_level
                FROM language_verifications
                WHERE applicant_id = ?
                ORDER BY is_verified DESC, language ASC
            `, [userId], (err, results) => {
        ...
        // Build the live profile response
        const liveProfile = {
            id: profile.id,
            firstName: profile.first_name,
            surname: profile.surname,
            ...
            profileVisibility: 'public', // Default to public for compatibility
            ...
            languages: languages,
            slug: profile.public_profile_url ? profile.public_profile_url.split('/').pop() : `${profile.first_name?.toLowerCase()}-${profile.surname?.toLowerCase()}` || 'user'
        };
