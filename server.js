...
app.use('/auth', authRoutes); // Add auth routes without /api prefix for backward compatibility
app.use('/api/application', applicationRoutes);
app.use('/api/application-enhanced', applicationEnhancedRoutes);
app.use('/api/documents', documentRoutes);
app.use('/api/profile', profileRoutes);
app.use('/api/cv', profileRoutes); // Alias profile routes as cv routes for backward compatibility
...
app.get('/pre-application', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'pre-application.html'));
});
...
// Profile edit page (for logged-in users)
...
// Export the Express app for Passenger (cPanel) to handle
module.exports = app;

// Only start the server when run directly (development/testing)
...
if (require.main === module) {
    ...
    app.listen(PORT, HOST, () => {
        ...
        console.log(`   In production, Passenger will handle the server startup`);
    });
} else {
    console.log('✅ App exported for Passenger - server startup handled by cPanel');
}
